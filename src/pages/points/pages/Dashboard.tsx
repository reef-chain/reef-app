...
function Dashboard() {
  ...
  const handleLogout = () => {
    analyticsFormo.track('points_logout', {
  ...
  const activeCampaigns = campaigns.filter(
    (c) => c.bootstrappingEligible || c.earlySznEligible || c.memeSznEligible
  ).length;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', padding: '1.5rem', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <Uik.Tabs
        ...
        options={NAV_TABS}
        onChange={(val) => {
          if (val === 'campaigns') navigate('/points/dashboard/campaigns');
        }}
      />
      ...
      <div style={{ marginTop: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 200px' }}>
            <Uik.Card title="Total Campaigns">
              <Uik.Text type="title" text={String(total)} />
            </Uik.Card>
          </div>
          <div style={{ flex: '1 1 200px' }}>
            <Uik.Card title="Active Campaigns">
              <Uik.Text type="title" text={String(activeCampaigns)} />
            </Uik.Card>
          </div>
        </div>
