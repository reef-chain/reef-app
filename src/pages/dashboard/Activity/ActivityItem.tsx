...
const TokenActivityItem = ({
  ...
  const { symbol, name, iconUrl } = token as NFT;
  const { nftId, mimetype } = token as NFT;
  ...
  const amount = useMemo(() => {
    const amt = balanceUtils.formatDisplayBalance(token.balance, 2);
    const prefixMap = {
      receive: "+",
      send: "-",
    };
  ...
  const activityPreviewIcon = useMemo(() => {
    const iconUrlIpfsResolved = iconUrl.startsWith("ipfs")
      ? getIpfsGatewayUrl(iconUrl.substring(7))
      : iconUrl;
    ...
    <>
      ...
      >
        <div className="activity-item__indicator">
          <Uik.Icon
            className="activity-item__indicator-icon"
            icon={faArrowDown}
          />
        </div>
        ...
        <div className="activity-item__content">
          <div className="activity-item__info">
            <div className="activity-item__title" title={title}>
              {title}
            </div>
